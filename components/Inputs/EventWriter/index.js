...
useEffect(()=>{
  ...
  const addEvent = async (arrImg) =>{

    console.log('ADD EVENT !')
    const value = await AsyncStorage.getItem('USER_EMAIL')
    if(value !== null){

        await eventBackend.CreateEvent(value,links,arrImg,verses,themeId,false,textEvent)
        .then(()=>{
          ...
          //-----------------------------
            ...
            let toast = Toast.show('Evenement ajouté', {
    ...
    return (
        <View style={{flex:1,flexDirection:"column",backgroundColor:'#fff',padding:0,alignItems:"center",padding:10,borderRadius:20,marginBottom:10}}>
            ...
            <View style={{flex:1,flexDirection:"row"}}>
                        <Text style={{fontSize:14,color:COLORS.green}}># Ajouter un événement</Text>
            </View>

            {showActivty&&
              <View style={{flex:1,flexDirection:"column",alignItems:"center"}}>
                  <ActivityIndicator size="large" color={COLORS.green} />
                  <Text style={styleFont.message}>Ajout en cours ...</Text>
              </View>
            }
